...
#
# ⚠️ 版位比例受**生图端能产出的宽高比**约束,不能只按分镜节奏挑。
# slot_sizes 会把每格的版位尺寸下发给 S4 单独出图,但 DGX 的 image-shim 底层是 ComfyUI 的
# ResolutionSelector 节点,只接受 8 个固定宽高比,最宽的一档是 21:9 ≈ 2.33
# (scripts/dgx-shims/image-shim.main.py 的 _ASPECT_RATIOS)。要一个比它更宽的版位,
# 拿回来的仍是 2.33,cover 再把多出来的高度裁掉——人物头部首当其冲。
# 2 格原先是上下两条整页宽横条(1920×540,比例 3.56),实测生成回来 2.33、再裁掉约 35%
# 高度,线上 f50b97f4 第 10 页重绘后人物头顶仍被切。改成左右分栏(960×1080,比例 0.89)
# 后落在 1:1 档附近,几乎不用裁。
# 3 格的上条(比例 3.23)同样越界,但它下面两格是 1.98、结构上换不成竖栏,暂留。
LAYOUTS: dict[int, list[tuple[float, float, float, float]]] = {
    1: [(0.0, 0.0, 1.0, 1.0)],
    2: [(0.0, 0.0, 0.5, 1.0), (0.5, 0.0, 1.0, 1.0)],
