...
## 七、平台适配性：NVIDIA DGX Spark + 开源模型 + 阶跃星辰模型
...
阶跃星辰提供的 `step-3.7-flash` 模型作为文本生成主路径之一，显著提升了剧本与分镜的推理质量；NVIDIA DGX Spark 的统一内存与本地 GPU 能力使“单机关闭环、无需外部云服务”成为可能。

### 7.1 Sehyo-Qwen3.5-35B-A3B-NVFP4模型

本项目选用 **Sehyo/Qwen3.5-35B-A3B-NVFP4** 作为 DGX Spark 本地部署的核心文本模型之一。该模型基于 Qwen3.5-35B-A3B MoE 架构量化而来，在保持原模型能力的同时，完整保留了 **MTP（Multi-Token Prediction）** 权重，可进一步结合 vLLM 的 **Speculative Decoding（投机解码）** 技术提升推理吞吐，是当前 Qwen3.5 系列中兼顾性能、成本与部署效率的高性价比方案。
...
本项目接入了 **阶跃星辰 Step-3.7-Flash** 大语言模型，作为云端部署的核心文本模型之一。Step-3.7-Flash 是阶跃星辰推出的新一代高性能推理模型，在保持较强综合能力的同时，针对推理速度、响应时延和部署成本进行了深度优化，能够为 AI Agent、多轮对话、代码生成、知识问答及内容创作等场景提供稳定、高效的模型服务。
