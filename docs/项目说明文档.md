...
| S1 剧本 | LLM | `hermes-agent`（编剧大师 skill，底层 `glm-5.2`） | `127.0.0.1:8642` |
| S2 分镜 | LLM | `hermes-agent`（导演大师 skill，底层 `glm-5.2`） | `127.0.0.1:8642` |
...
> **说明**：
>
> - S1–S2 的 LLM 环节全部走本地 Hermes Agent（`127.0.0.1:8642`），由 Hermes 内部托管的 `glm-5.2` 模型配合编剧大师 / 导演大师 skill prompt 生成，受电影业界创作方法论约束。
...
在本项目中，Step-3.7-Flash 主要承担景点故事搜索和来源甄别、角色特性生成等核心职责。得益于其优秀的中文理解能力以及工具调用能力，模型能够有效支撑多 Agent 工作流中的任务拆解、上下文推理和结果生成，为整个系统提供稳定可靠的智能中枢能力。
