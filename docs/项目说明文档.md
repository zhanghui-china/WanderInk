...
项目引入 **Hermes agent** 承载“编剧大师”与“导演大师”两个专业 skill：

- **编剧大师 skill**：负责 S0 传说检索与 S1 剧本改编，输出更符合影视叙事结构（冷开场、起承转合、主角 ≤4）的剧本；
- **导演大师 skill**：负责 S2 分镜拆分，输出带镜头语言（全景/中景/特写、光线、氛围）与情绪标签的分镜表。

Hermes 的接入为流水线注入了电影行业知识，使生成内容从“通用 LLM 出文本”升级为“受专业创作方法论约束的结构化产出”。
...
```mermaid
flowchart TB
    subgraph FE["🖥️ 前端层 · React + Vite + Tailwind"]
        UI["Web 界面<br/>新建作品 / 队列 / 详情 / 三视图 / 逐页预览<br/>重绘 / 重配音 / 下载 PDF·图片包"]
        CFG["⚙️ 配置面板<br/>全局默认 + S0–S5 按环节覆盖<br/>运行时生效 · 持久化 config.json"]
        UI --> CFG
    end

    subgraph BE["⚙️ 后端层 · FastAPI + ThreadPool"]
        API["API 层<br/>POST /api/projects · GET /api/projects<br/>GET /api/queue · POST /cancel<br/>后台线程跑管线 · 前端轮询"]
        CLI["CLI 层 · shanhai (Typer)<br/>new / pick / step / run / status"]
        STEPS["流水线步骤 steps/*<br/>S0 传说 → S1 剧本 → S2 分镜 → S3 角色 → S4 页面 → S5 音频 → S6 合成"]
        PROJ[("📦 Project 聚合根<br/>project.json<br/>原子写 · 断点续跑 · 唯一事实源")]
        STORE["store.py<br/>create / save / load · 写 .tmp 后 os.replace"]
    end

    subgraph PROV["🔌 Provider 层 · OpenAI 兼容协议 · 可插拔"]
        LLM["LLMClient<br/>chat / structured"]
        IMG["ImageClient<br/>images_api / chat_api"]
        TTS["TTSClient<br/>synthesize"]
        MUS["MusicClient<br/>generate"]
        GUARD["🔒 local_backend_guard<br/>127.0.0.1 端点全局单并发<br/>避免 DGX 共享 GPU 抢卡"]
        LLM --> GUARD
        IMG --> GUARD
        TTS --> GUARD
        MUS --> GUARD
    ...
    subgraph DGX["🖥️ NVIDIA DGX Spark · GB10 · 119GB 统一内存 · 分时加载"]
        direction LR
        OLLAMA["Ollama :11434<br/>glm-4.7-flash / qwen3.5:122b<br/>~40GB 常驻"]
        HERMES["Hermes Agent :8642<br/>编剧大师 skill / 导演大师 skill<br/>内部托管 LLM + skill prompt"]
        COMFY["ComfyUI :8188<br/>Qwen-Image-Edit-2511 / Krea2 Turbo<br/>shanhai-image :8091 桥接"]
        COSY["CosyVoice2 :8090<br/>0.5B · PyTorch nightly cu128<br/>GB10 sm_121 适配"]
        ACE["ACE-STEP 1.5 XL :8092<br/>shanhai-music 桥接<br/>纯器乐 BGM 生成"]
    end

    subgraph CLOUD["☁️ 外部云端服务（可选 · 按配置切换）"]
        TUZI["tu-zi API<br/>gpt-image-2 图像"]
        STEPFUN["Stepfun step-3.7-flash<br/>文本生成"]
    end

    subgraph OUT["🎬 输出层"]
        FF["FFmpeg + PIL 排版<br/>Ken Burns · 字幕分层 · 水印<br/>xfade · loudnorm · BGM 混音"]
        MP4[("final.mp4<br/>1080p · h264+aac<br/>带片头片尾 · AI 水印 · 来源标注")]
        PDF[("PDF / 图片包 ZIP")]
    end

    %% 前后端连接
    UI -->|"HTTP / 轮询"| API
    CFG -->|"PUT /api/config"| API
    API --> STEPS
    CLI --> STEPS
    STEPS -->|"读写"| STORE
    STORE --> PROJ
    ...
    %% 步骤调用 Provider
    STEPS -->|"S0/S1/S2/S3 文本"| LLM
    STEPS -->|"S3/S4 图像"| IMG
    STEPS -->|"S5 配音"| TTS
    STEPS -->|"S5 配乐"| MUS

    %% Provider 到服务
    LLM -->|"按环节配置<br/>S0/S1 → Hermes<br/>S2/S3 → Ollama/Stepfun"| HERMES
    LLM --> OLLAMA
    LLM -.->|"可选"| STEPFUN
    IMG --> COMFY
    IMG -.->|"可选"| TUZI
    ...
    classDef provider fill:#f3e8ff,stroke:#7c3aed,stroke-width:2px
    classDef dgx fill:#dcfce7,stroke:#16a34a,stroke-width:2px
    classDef cloud fill:#fce7f3,stroke:#db2777,stroke-width:2px
    classDef output fill:#fed7aa,stroke:#ea580c,stroke-width:2px

    class UI,CFG frontend
    class API,CLI,STEPS,PROJ,STORE backend
    class LLM,IMG,TTS,MUS,GUARD provider
    class OLLAMA,HERMES,COMFY,COSY,ACE dgx
    class TUZI,STEPFUN cloud
    class FF,MP4,PDF output
```

> **图例说明**：
> - 蓝色为前端层，黄色为后端编排层，紫色为可插拔 Provider 层，绿色为 DGX Spark 本地服务，粉色为可选云端服务，橙色为输出层。
> - `Project` 聚合根是唯一事实源，所有步骤读写同一个 `project.json`。
> - `local_backend_guard` 自动识别本地端点并加全局锁，保证 DGX 共享 GPU 不被多任务抢卡。
> - 虚线表示可选路径（如云端图像、Stepfun 文本），实线为默认路径。
> - Hermes 内部托管 LLM 与 skill prompt，WanderInk 只通过 `hermes-agent` 模型名调用。
...
| 环节 | 用途 | 生效模型 | 端点 |
|---|---|---|---|
| S0 传说 | LLM | `hermes-agent`（编剧大师） | `127.0.0.1:8642`（DGX 本地） |
| S1 剧本 | LLM | `hermes-agent`（编剧大师） | `127.0.0.1:8642`（DGX 本地） |
| S2 分镜 | LLM | `step-3.7-flash` | `api.stepfun.com`（阶跃星辰云端） |
| S3 角色三视图 | LLM + 图像 | `step-3.7-flash` + `gpt-image-2` | stepfun 云端 + `api.tu-zi.com`（tu-zi 云端） |
| S4 漫画页 | 图像 | `gpt-image-2` | `api.tu-zi.com`（tu-zi 云端） |
| S5 配音/BGM | TTS + 音乐 | `cosyvoice2` + `ace-step-v1.5xl` | 都在 `127.0.0.1`（DGX 本地 shim） |

> **说明**：
> - S0/S1 走本地 Hermes 编剧大师 skill，由 Hermes 内部托管的 LLM 配合 skill prompt 生成，受电影业界创作方法论约束。
> - S2/S3 的 LLM 走 Stepfun 阶跃星辰 `step-3.7-flash` 云端，由赛事赞助方提供。
> - S3 三视图的图像与 S4 逐页图像走 tu-zi 云端 `gpt-image-2`；三视图在 M0 门禁中以 100% 零身份漂移通过。
...
| Agent | 职责 | 底层能力 / 端点 |
|---|---|---|
| StoryAgent | 景区 → 历史故事概要（S0） | Hermes 编剧大师 skill（`127.0.0.1:8642`） |
| ScriptAgent | 故事 → 剧本（S1） | Hermes 编剧大师 skill（`127.0.0.1:8642`） |
| DirectorAgent | 剧本 → 分镜（S2） | Stepfun `step-3.7-flash`（`api.stepfun.com`） |
| CharacterAgent | 剧本 → 人物小传与道具描述（S3） | Stepfun `step-3.7-flash`（`api.stepfun.com`） |
...
- **按环节配置覆盖**：Web 配置面板支持“全局默认 + S0–S5 按环节覆盖”，例如 S0/S1 走 Hermes 编剧 skill，S2 走本地 Ollama，图像走云端或本地 ComfyUI，无需重启即可生效。
