...
**WanderInk（中文名：“漫游墨绘”）** 是一款面向景区文化 IP 开发与数字内容生产的端到端多模态 AI 创作系统。

英文名称 **WanderInk** 由 *Wander* 与 *Ink* 两部分组成。其中，*Wander* 寓意探索、漫游与文化寻迹，代表用户在 AI 的帮助下穿越历史与地域，探索景区背后的文化故事；*Ink* 象征笔墨、绘画与传统艺术表达，体现系统通过 AI 技术将文化内容转化为视觉艺术作品。

中文名称“漫游墨绘”融合了探索之旅与传统笔墨意境，既体现了景区文化体验的沉浸感，也表达了人工智能赋能传统文化创作的新形态。

### 1.2 项目目标

WanderInk 致力于打造面向文化旅游场景的 **AI 原生多模态内容生产平台**。

用户只需输入一个景区名称，系统即可自动完成：
...
最终输出一部具备 **1080P 高清画质、完整剧情叙事、配音配乐效果的有声连环画短视频作品**。
...
WanderInk 源于第一届 Spark 黑客松项目 ** [SparkScroll](https://github.com/zhanghui-china/SparkScroll)** 的技术积累与产品理念，并在本届项目中完成了从“AI 漫画生成工具”向“文化 IP 多模态智能创作平台”的全面升级。

相比上一代 SparkScroll 项目，本项目主要实现了以下四个方面的技术演进：

**（1）从单一视觉生成升级为多模态内容生产**

第一代 SparkScroll 主要聚焦于连环画生成，而 WanderInk 进一步融合：
...
实现从故事文本到完整视听作品的端到端生成。

**（2）从通用大模型生成升级为专业化创作 Agent**

传统大模型虽然具备文本生成能力，但在影视内容生产领域仍存在剧情结构不足、镜头语言欠缺、角色一致性较弱等问题。
...
封装为多个专业化 AI Agent / Skill，通过多 Agent 协同完成内容生产，使生成结果更加符合影视和漫画创作规范。

**（3）从全自动黑盒生成升级为可控创作工作流**

针对 AI 内容生成过程中“不可控、难修改”的问题，WanderInk 构建了可干预的创作工作台：
...
进行人工审核、调整和重新生成，实现“AI 自动创作 + 人工创意控制”的协同模式。

**（4）从单机原型升级为多用户 Web 应用平台**
...
系统把景区故事创作拆分为 **S0–S6 七个顺序步骤**，覆盖文本、图像、语音、音乐、视频五种模态，单条流水线（管线）即可从“景区名”直达“MP4 成片”，无需人工切换工具：
...
项目引入 **Hermes Agent**（底层配置 `Step-3.7-Flash` 模型）承载“山音超级编剧大师”与“山音超级导演大师”两个专业 skill，覆盖 S1–S3 全部文本生成环节：

- **山音超级编剧大师 skill**：负责 S1 剧本改编，输出更符合影视叙事结构（冷开场、起承转合、主角 ≤3）的剧本；
- **山音超级导演大师 skill**：负责 S2 分镜拆分与 S3 角色特征提取，输出带镜头语言（全景/中景/特写、光线、氛围）与情绪标签的分镜表。
...
WanderInk  系统采用 **"Supervisor + 顺序管线 + 可插拔 Provider"** 的三层架构设计：
...
> - S0–S3 文本环节全部走 Hermes 编剧/导演 skill（底层 `Step-3.7-Flash`）；图像走 tu-zi 云端 gpt-image-2。
...
| `shanhai-image` | 8091  | Qwen-Image-Edit图像服务（OpenAI 兼容） |
...
WanderInk系统是一个**专业级、可干预、可部署**的多模态创意系统，其技术架构体现了以下核心价值：
...
- **按环节配置覆盖**：Web 配置面板支持“全局默认 + S0–S5 按环节覆盖”，例如 S1–S2 全部走 Hermes 编剧/导演 skill（底层 `Step-3.7-Flash`），图像走云端或本地 ComfyUI，无需重启即可生效。
...
#### 7.1.3 ACE Studio 与 StepFun（阶跃星辰）**ACE-Step 1.5 XL** 模型
...
#### 7.2.4 剧本与分镜生成服务——基于 Hermes Agent Framework 与山音 Skill 服务的多 Agent 创作架构

在内容创作环节，本项目采用 **Hermes Agent Framework** 作为多 Agent 协同编排框架，并将 **山音超级编剧大师** 、 **山音超级导演大师** 封装为 Hermes 体系下的专业化 Skill 服务，通过 HTTP API 方式向上层 Agent 提供剧本生成和分镜生成能力，构建了面向 AI 内容生产场景的智能创作工作流。

**Hermes** 是面向大模型时代构建智能体（Agent）应用的轻量化编排框架，其核心理念是通过 **Agent + Skill + Tool** 的模块化架构，将复杂任务拆解为多个具备专业能力的服务模块，并通过统一的任务调度、上下文管理和能力调用机制，实现多个智能体之间的协同工作。

随着大语言模型能力不断增强，单一 LLM 已难以满足复杂业务场景需求。尤其是在影视、漫画、短剧等内容生产领域，一个完整创作流程通常涉及故事策划、剧本创作、导演分镜、美术设计、音乐制作等多个专业环节。如果仅依赖单一模型进行端到端生成，容易出现剧情逻辑不稳定、角色设定漂移、镜头语言不足等问题。因此，本项目采用 Hermes 作为智能编排层，将编剧、导演的专业Skill，通过 Agent 动态调用完成复杂创作任务。

[山音超级编剧大师]: https://github.com/Shanyin-ai/shanyin-screenwriting-master

 作为 Hermes 的剧本创作 Skill，主要负责故事策划、世界观构建、人物设定以及剧本生成等任务。

该 Skill 通过 HTTP API 向 Hermes 提供标准化调用接口，使 Agent 能够根据用户输入的主题、类型、背景设定以及创作要求，调用编剧能力完成：
...
通过将编剧能力封装为独立 Skill，系统无需将复杂创作逻辑固化在主 Agent 中，而是可以根据任务需求动态调用专业能力，实现创作能力模块化和服务化。

[山音超级导演大师]: https://github.com/Shanyin-ai/shanyin-director-master

在完成剧本生成后，Hermes Agent 会进一步调用 **山音超级导演大师** Skill，将文学化剧本转换为适用于视觉生成的导演级分镜数据。
...
通过 HTTP 服务接口输出结构化剧本、分镜内容，为后续 Qwen-Image-Edit-2511 图像生成模型提供精准的 Prompt 和场景控制信息。

相比直接通过大语言模型生成图片描述，本方案增加了“编剧 → 导演 → 视觉生成”的专业化转换层，使 AI 生成内容更加符合影视创作流程，提高了剧情一致性、镜头连续性以及角色表现能力。

本项目所采用的 **山音超级编剧大师（Shanyin Screenwriting Master）** 和 **山音超级导演大师（Shanyin Director Master）** 均为开源软件项目。项目作者 [山音](https://github.com/Shanyin-ai)  是国内知名 AIGC 艺术创作者、独立导演及编剧，在 AI 内容创作领域具有丰富的实践经验，曾获得 **2025 年初心榜十大年度 AIGC 人物、瓦卡奖中国 AI 视觉创意头部作者、非凡大赏年度 AI CREATOR 100 创作者** 等荣誉，并参与深圳 AIGC 超创实验室、郎园 AI 超创生态矩阵等创新生态建设。

其长期围绕 AIGC 艺术创作、智能内容生产流程和 AI 创意工具研发开展探索，并作为多家主流 AI 创作平台的创作者代表，持续推动 AI 技术在影视、视觉艺术和内容生产领域的应用。

*项目团队在此感谢 **@山音** 开源并贡献的专业化创作工具，为本项目在智能剧本生成、导演分镜规划以及 AI 内容生产流程优化方面提供了重要技术支撑。借助这些优秀的开源能力组件，本项目能够进一步完善多 Agent 协同创作体系，加速 AI 原生内容生产应用的探索与实践。*
...
在景区中输入景区名称，作品已经内置了全国文旅提供的全部5A级景区359个，比如你输入“华山”，会出现以下景区：

![page_01](../samples/opr/002.png)

此时：

- 选择需要生成的视频时长：1分钟、3分钟、5分钟
- 选择视频的受众：大众、儿童
- 选择视频基调：温情、奇幻、悬疑
- 选择画风：国风水墨、儿童绘本、现代插画
- 选择音色：男声、女声
- 选择语速：0.8、1.0、1.2
- 选择是否启用 山音超级编剧大师Skill生成剧本，山音超级导演大师Skill生成分镜。
- 如果景区没找到相关的故事，用户还可以自己准备相关的故事。

点击【开始生成】，WanderInk就开始工作了。
...
当本节点完成时，下面会显示生成的故事内容，信息来源。

![page_01](../samples/opr/004.png)

### 8.3 S1剧本生成

当本节点完成时，剧本已生成，且人物设定已完成，会展示如下信息，显示故事中有白素贞、许仙、法海和小青四个人物。

![page_01](../samples/opr/005.png)

### 8.4 S2分镜生成

当本节点完成时，分镜文本已生成，会展示如下9页分镜信息：
...
## 十、本项目与第一届 Spark 黑客松 SparkScroll 的区别
...
| 评审维度 | 权重 | 本项目对应内容 |
|---|---|---|
| 项目实用性、行业落地价值与技术创新性 | 25% | 解决中小景区 IP 开发“周期长、成本高”痛点；端到端有声连环画方案具备行业首创性；充分利用 DGX Spark 统一内存实现单机闭环 |
...
| 平台适配性 | 15% | DGX Spark 128GB 统一内存分时加载；vllm 本地 LLM；ComfyUI 图像管线；Qwen3-TTS 语音；Stepfun ACE-STEP 音乐；Stepfun step-3.7-flash 文本模型 |
